...
- **Note**: For these we'll use "reasonable" costs for bus-stops and 
  bus-transfers: counting each bus stop as equivalent to traveling 0.5km, and 
  each bus transfer as traveling 5km. With an average speed of 20km/h, that's 
  equivalent to saying each stop is for 90s and each bus transfer takes 15 
  minutes; not exact, but roughly the correct order of magnitude.
